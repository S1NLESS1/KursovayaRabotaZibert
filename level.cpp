...
bool hero::behavior(sf::Event event, Game* game, int nof_object) {
	...
	*CurrentAttac += 0.002 * time; // кд на атаку
...
Game::Game(Level *level) {
	if (!font.loadFromFile("Fonts/FontOver.otf")) std::cout << "ERROR! Couldn't load a font" << std::endl; // шрифт
...
void Game::game_draw(class sf::RenderWindow* window, class Level level, sf::FloatRect visibleArea, int nof_player, std::string mode) {
	...
	sf::View generalView; // общее окно
	if (mode == "SINGLE") {
		for (int i = 0; i < 2; i++) {
			...
			playerView[i].setCenter(pos.x, pos.y); // обновить центрирование экрана
		}
		std::vector <sf::View> playerView;
		playerView.push_back(sf::View());
		playerView.push_back(sf::View());
		...
		generalView.setCenter((playerBody[0]->GetPosition().x + playerBody[1]->GetPosition().x) / 2, (playerBody[0]->GetPosition().y + playerBody[1]->GetPosition().y) / 2); // обновить центрирование экрана
	}
